...
final class Validator
{
   ...
   */
  public function set(string $name, mixed $value, ?string $custom_name = null)
  {
    $this->_data['name'] = $custom_name ?? $name;
   ...
   * @return Validator The self instance
   */
  public function min_length(int $length, $inclusive = false)
  {
   ...
   * @return Validator The self instance
   */
  public function max_length(int $length, $inclusive = false)
  {
    $verify = ($inclusive === true ? strlen($this->_data['value']) <= $length : strlen($this->_data['value']) < $length);
    if (!$verify) {
   ...
   * @return Validator The self instance
   */
  public function between_length(int $min, int $max)
  {
  ...
  {
    if (!$this->validateCnpj($this->_data['value'])) {
  ...
  {
    if (!$this->validatePhone($this->_data['value'])) {
  ...
  public function is_card_number()
  {
    $verify = true;

    $input = preg_replace('/\D/', '', $this->_data['value']);

    if (mb_strlen($input) != 16)
      $verify = false;

    if (!$verify) {
      $this->set_error(sprintf($this->_messages['is_card_number'], $this->_data['value']));
    }
    return $this;
  }
